...
#'
#' @keywords internal
build_train_list_Macosko <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   ...
   cl.df$class_label   = cl.df$subclass_label # cl.df$Level1_label
   ...
   # WholeBrain Parquet
   dsFN = file.path(WBDIR, "de_parquet")
   pairsFN = file.path(WBDIR, "pairs.parquet")
   cl.binFN = file.path(WBDIR, "cl.bin.CK.rda")
   all.pairs = read_parquet(pairsFN)
   load(cl.binFN)
   ...
   train.cl.dat = train.cl.dat[intersect(rownames(train.cl.dat),select.markers),]
...
#'
#' @keywords internal
build_train_list_Macosko_MBHB <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   ...
   dsFN = file.path(WBDIR, "de_parquet")
   pairsFN = file.path(WBDIR, "pairs.parquet")
   cl.binFN = file.path(WBDIR, "cl.bin.rda")
   all.pairs = read_parquet(pairsFN)
   load(cl.binFN)
   select.markers = rownames(train.cl.dat)
   if (is.na(query.genes)) query.genes = select.markers
...
#'
#' @keywords internal
build_train_list_Macosko_MBHB_dasi <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_CorticoSpinal_MBHB <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_Multiome_10X_MBHB <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_Siletti_AIT21_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_Siletti_AIT21 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_Macosko_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21.5_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   ...
   select.markers = intersect(select.markers, rownames(train.cl.dat))
   if (is.na(query.genes)) query.genes = select.markers
   if (is.list(pre.train.list)) {
      select.markers = intersect(pre.train.list$all.markers, query.genes)
   } else {
...
#'
#' @keywords internal
build_train_list_WB21.4_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21.3_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   cl.df = as.data.frame(cl.df)
   #cl.df$cl = as.character(cl.df$cl)
   #cl.df$subclass_label = as.character(cl.df$subclass_label)
   #cl.df$class_label = as.character(cl.df$class_label)
...
#'
#' @keywords internal
build_train_list_WB21.5 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   ...
   all.pairs = read_parquet(pairsFN)
   load(file.path(WBDIR, "cl.bin.rda"))
   load(file.path(WBDIR, "select.markers.curated.rda")) # pooled
   select.markers = select.markers.curated
   if (is.na(query.genes)) query.genes = select.markers
...
#'
#' @keywords internal
build_train_list_WB21.4 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21.3 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21.2_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21.2 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21.1 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.5322.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   ...
   tmp=load(file.path(TaxDir, "cl.means.8.rda"))
   train.cl.dat = cl.means.8
   common.cluster = intersect(colnames(train.cl.dat), cl.df$cl)
   if (length(rm.cl)>0) {
      common.cluster = setdiff(common.cluster, rm.cl)
   }
...
#'
#' @keywords internal
build_train_list_WB21_root <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.freeze.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   ...
   tmp=load(file.path(TaxDir, "cl.means.freeze.rda"))
   train.cl.dat = cl.means
   common.cluster = intersect(colnames(train.cl.dat), cl.df$cl)
   if (length(rm.cl)>0) {
      common.cluster = setdiff(common.cluster, rm.cl)
...
#'
#' @keywords internal
build_train_list_WB21.1_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.AIT21.1.rda"))
   cl.df = as.data.frame(cl.df)
   #cl.df$cl = as.character(cl.df$cl)
   #cl.df$subclass_label = as.character(cl.df$subclass_label)
   #cl.df$class_label = as.character(cl.df$class_label)
   ...
   tmp=load(file.path(TaxDir, "cl.means.AIT21.1.rda"))
   train.cl.dat = cl.means
...
#'
#' @keywords internal
build_train_list_WB21_root_curated <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.freeze.230815.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   cl.df = as.data.frame(cl.df)
   #cl.df$cl = as.character(cl.df$cl)
   ...
   tmp=load(file.path(TaxDir, "cl.means.freeze.curated.rda"))
   train.cl.dat = cl.means
...
#'
#' @keywords internal
build_train_list_WB21_root_curated_NN <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c(), NNmarker=FALSE)
{
   tmp = load(file.path(TaxDir, "cl.df.freeze.230815.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21_level5 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=5, TaxFN, rm.cl=c())
{
...
#'
#' @keywords internal
build_train_list_WB21_root_curated_MBHB <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB21.1.root <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.5322.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   ...
   load(file.path(TaxDir, "df.extracted_ensembl_ncbi_matched_genes.rda"))
   common.gene = intersect(df$source_index, rownames(train.cl.dat))
   gdx = match(common.gene, rownames(train.cl.dat))
   if(any(is.na(gdx))) {print("somethingis wrong with genes") ; browser()}
   train.cl.dat = train.cl.dat[gdx, common.cluster]
   cl.df = cl.df %>% filter(cl %in% common.cluster)
   ...
   load(file.path(WBDIR, "cl.bin.rda"))
   load(file.path(WBDIR, "select.markers.rda")) # pooled
   select.markers = intersect(select.markers, common.gene)
   if (is.na(query.genes)) query.genes = select.markers
   if (is.list(pre.train.list)) {
      select.markers = intersect(pre.train.list$all.markers, query.genes)
   } else {
...
build_train_list_WB21 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.freeze.230815.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
build_train_list_WB21_top5 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.freeze.230815.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
build_train_list_WB21_top10 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.freeze.230815.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_5_1 <- function(pre.train.list=NA, query.genes=NA, TrainDir, TaxDir, prefix="", nlevel=3, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TrainDir, "cl.df.rda"))
   #cl.df = cl.df.clean
   cl.df$cl = as.character(cl.df$cl)
   ...
   cl.df$class_label = gsub("/", "_", cl.df$class_label)
   cl.df$Level2_label = cl.df$subclass_label
   cl.df$Level1_label = cl.df$class_label
   tt21 = table(cl.df$subclass_label, cl.df$class_label)
   tt21.major = colnames(tt21)[apply(tt21, 1, function(x){which.max(x)})]
   names(tt21.major) = rownames(tt21)
...
#'
#' @keywords internal
build_train_list_19 <- function(pre.train.list=NA, query.genes=NA, TrainDir, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   cl.df = as.data.frame(cl.df)
   cl.df$cl = as.character(cl.df$cl)
   ...
   cl.df$subclass_label = cl.df$Level2_label
   cl.df$neighborhood   = tt21.major[cl.df$Level2_label]# cl.df$Level1_label
   cl.df$class_label    = tt21.major[cl.df$Level2_label]# cl.df$Level1_label

   tmp=load(file.path(TaxDir, "cl.means.core.rda"))
   train.cl.dat = cl.means.core 
   colnames(train.cl.dat) = gsub("\\.0$", "", colnames(train.cl.dat))
   common.cluster = intersect(colnames(train.cl.dat), cl.df$cl)
   if (length(rm.cl)>0) {
      common.cluster = setdiff(common.cluster, rm.cl)
   }
...
#'
#' @keywords internal
build_train_list_18_1 <- function(pre.train.list=NA, query.genes=NA, TrainDir, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   tmp = load(file.path(TaxDir, "cl.df.rda"))
   #cl.df = cl.df.clean
   cl.df$cl = as.character(cl.df$cl)
...
#'
#' @keywords internal
build_train_list_WB17_cl5196 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   CLSDIR = WBDIR
   tmp = load(file.path(TaxDir, "cl.rda"))
   cl.df$cl = as.character(cl.df$cl)
...
build_train_list_WBint <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   ...
   cl.df$class_label   = cl.df$Level1_label
