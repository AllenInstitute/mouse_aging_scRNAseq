...
library(arrow) ; library(Matrix) ; library(dplyr)
...
run_mapping_on_taxonomy <- function(query.dat,
                             ...
                             subsample_pct=0.9, top.n.genes=15, div_thr=1, rm.clusters=NULL, NNmarker=NULL,
                             flag.serial=TRUE, flag.parallel.tmp=FALSE, flag.fuzzy=FALSE )
{
   ...
   train.list = build_train_list_on_taxonomy ( TaxFN, Taxonomy, TaxHome=TaxHome,
                                               ...
                                               top.n.genes=top.n.genes, rm.cl=rm.clusters, NNmarker )
...
build_train_list_on_taxonomy <- function ( TaxFN=NA, Taxonomy, 
                                           ...
                                           n.group.genes=3000, rm.cl=NULL, NNmarker=NULL )
{
   AVAIL_TAXONOMIES = c( "Human_MTG_Benchmarking_500",
                   "Human_MTG_Benchmarking",
                   "CorticoSpinal_MBHB",
                   "Multiome_10X_MBHB",
                   "Macosko", #AIT21.0_root_curated - IMN - OB
                   "Macosko_root_curated", #AIT21.0_root_curated - IMN - OB
                   "Macosko_MBHB", #AIT21.0_root_curated - IMN - OB
                   "Macosko_MBHB_dasi", #AIT21.0_root_curated - IMN - OB
                   "Siletti_AIT21_root_curated", #AIT21.0_root_curated - IMN - OB
                   "Siletti_AIT21", #AIT21.0_root_curated - IMN - OB
                   "AIT21.5_mouse", #AIT21.0_mouse - IMN - OB
                   "AIT21.5_root_curated", #AIT21.0_root_curated - IMN - OB
                   "AIT21.4_mouse", #AIT21.0_mouse - TH
                   "AIT21.4_root_curated", #AIT21.0_root_curated - TH
                   "AIT21.3_mouse", #AIT21.0_mouse - immature subclass
                   "AIT21.3_root_curated", #AIT21.0_root_curated - immature subclass
                   "AIT21.2_mouse", #WB 5322 mPFC_PL_ILA_ORB
                   "AIT21.2_root_curated", #WB 5322 mPFC_PL_ILA_ORB
                   "AIT21.1_mouse", #WB 5322 with ensemble ncbi matching genes	
                   "AIT21.0_level5", #WB 5322	
                   "AIT21.0_root_curated", #WB 5322	
                   "AIT21.0_root_curated_woIMM", #WB 5322	
                   "AIT21.0_root_curated_MBHB", #WB 5322	
                   "AIT21.0_root_mouse", #WB 5322	
                   "AIT21.0_root", #WB 5322	
                   "AIT21.0_mouse_top5", #WB 5322	
                   "AIT21.0_mouse_top10", #WB 5322	
                   "AIT21.0_mouse_woIMM", #WB 5322	
   ...
   if (grepl('root', Taxonomy)) {
      if (is.null(NNmarker)) {
         load("/allen/programs/celltypes/workgroups/rnaseqanalysis/changkyul/Mapping_On_Taxonomy/ck-scrattch-mapping/R/mfish.genes.curated.rda")
      } else {
        load(paste0("/allen/programs/celltypes/workgroups/rnaseqanalysis/changkyul/Mapping_On_Taxonomy/ck-scrattch-mapping/R/mfish.genes.curated.NNmarker", NNmarker, ".rda"))
      }
      root.markers = mfish.genes.curated
      if (length(query.genes)>1) root.markers = intersect(mfish.genes.curated, query.genes)
   } else {
      root.markers = NULL
   }
   if (is.na(Taxonomy)) {
      ...
      if (Taxonomy=="AIT21.4_root_curated" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.4_root_curated"
      }
      if (Taxonomy=="AIT21.3_root_curated" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.3_root_curated"
      }
      if (Taxonomy=="Siletti_AIT21" ) { 
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Siletti_AIT21"
         TrainDir = TaxDir
      }
      if (Taxonomy=="Siletti_AIT21_root_curated" ) { 
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Siletti_AIT21_root_curated"
         TrainDir = TaxDir
      }
      if (Taxonomy=="Multiome_10X_MBHB" ) { 
         TrainDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Multiome_10X_MBHB"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Multiome_10X_MBHB"
      }
      if (Taxonomy=="CorticoSpinal_MBHB" ) { 
         TrainDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/CorticoSpinal_MBHB"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/CorticoSpinal_MBHB"
      }
      if (Taxonomy=="Macosko_MBHB_dasi" ) { 
         TrainDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Macosko_MBHB_dasi"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Macosko_MBHB_dasi"
      }
      if (Taxonomy=="Macosko_MBHB" ) { 
         TrainDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Macosko_MBHB"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Macosko_MBHB"
      }
      if (Taxonomy=="Macosko_root_curated" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/10X_analysis/wholebrain_broad/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Macosko_root_curated"
      }
      if (Taxonomy=="Macosko" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/10X_analysis/wholebrain_broad/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/Macosko"
      }
      if (Taxonomy=="AIT21.5_root_curated" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.5_root_curated"
      }
      if (Taxonomy=="AIT21.2_root_curated" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.2_root_curated"
      }
      if (Taxonomy=="AIT21.5_mouse" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.5_mouse"
      }
      if (Taxonomy=="AIT21.4_mouse" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.4_mouse"
      }
      if (Taxonomy=="AIT21.3_mouse" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.3_mouse"
      }
      if (Taxonomy=="AIT21.2_mouse" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.2_mouse"
      }
      if (Taxonomy=="AIT21.1_mouse" ) { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.1_mouse"
      }
      if (Taxonomy=="AIT21.1_root_curated") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.1_root_curated"
      }
      if (Taxonomy=="AIT21.0_root_curated") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_root_curated"
      }
      if (Taxonomy=="AIT21.0_level5") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT215_root_curated"
         nlevel = 5
      }
      if (Taxonomy=="AIT21.0_root_curated_woIMM") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_root_curated_woIMM"
      }
      if (Taxonomy=="AIT21.0_root_curated_MBHB") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_root_curated_MBHB"
      }
      if (Taxonomy=="AIT21.0_root") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_root"
      }
      if (Taxonomy=="AIT21.0_mouse_woIMM") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_mouse_woIMM"
      }
      if (Taxonomy=="AIT21.0_mouse") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_mouse"
      }
      if (Taxonomy=="AIT21.0_mouse_top5") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_mouse_top5"
      }
      if (Taxonomy=="AIT21.0_mouse_top10") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/yzizhen/joint_analysis/wb/"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT21.0_mouse_top10"
      }
      ...
      if (Taxonomy=="AIT5.1_CTX_HPF") { 
         TrainDir = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT5.1_CTX_HPF"
         TaxDir   = "/allen/programs/celltypes/workgroups/rnaseqanalysis/shiny/Taxonomies/AIT5.1_CTX_HPF"
      }
...
#print("here") ; browser()
      if (file.exists(TaxFN)) {
         ...
         if (Taxonomy =="Siletti_AIT21") train.list = build_train_list_Siletti_AIT21( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="Siletti_AIT21_root_curated") train.list = build_train_list_Siletti_AIT21_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="Multiome_10X_MBHB") train.list = build_train_list_Multiome_10X_MBHB( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="CorticoSpinal_MBHB") train.list = build_train_list_CorticoSpinal_MBHB( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="Macosko") train.list = build_train_list_Macosko( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="Macosko_MBHB") train.list = build_train_list_Macosko_MBHB( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="Macosko_MBHB_dasi") train.list = build_train_list_Macosko_MBHB_dasi( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="Macosko_root_curated") train.list = build_train_list_Macosko_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.5_root_curated") train.list = build_train_list_WB21.5_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.5_mouse") train.list = build_train_list_WB21.5( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.4_root_curated") train.list = build_train_list_WB21.4_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.4_mouse") train.list = build_train_list_WB21.4( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.3_root_curated") train.list = build_train_list_WB21.3_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.3_mouse") train.list = build_train_list_WB21.3( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.2_root_curated") train.list = build_train_list_WB21.2_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.2_mouse") train.list = build_train_list_WB21.2( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.1_mouse") train.list = build_train_list_WB21.1( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.1_root_curated") train.list = build_train_list_WB21.1_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN, rm.cl=rm.cl)
         if (Taxonomy =="AIT21.0_level5") train.list = build_train_list_WB21_level5( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN, rm.cl=rm.cl)
         if (Taxonomy =="AIT21.0_root_curated_woIMM") train.list = build_train_list_WB21_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN, rm.cl=rm.cl)
         if (Taxonomy =="AIT21.0_root_curated_MBHB") train.list = build_train_list_WB21_root_curated_MBHB( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN, rm.cl=rm.cl)
         if (Taxonomy =="AIT21.0_root_curated") train.list = build_train_list_WB21_root_curated( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN, rm.cl=rm.cl )
         if (Taxonomy =="AIT21.0_root") train.list = build_train_list_WB21_root( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.0_mouse_woIMM") train.list = build_train_list_WB21( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN, rm.cl=rm.cl)
         if (Taxonomy =="AIT21.0_mouse") train.list = build_train_list_WB21( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN, rm.cl=rm.cl)
         if (Taxonomy =="AIT21.0_mouse_top5") train.list = build_train_list_WB21_top5( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT21.0_mouse_top10") train.list = build_train_list_WB21_top10( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         ...
         if (Taxonomy=="AIT9.LO10_mouse")  train.list = build_train_list_FBLO10( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         if (Taxonomy =="AIT5.1_CTX_HPF") train.list = build_train_list_5_1( 
                                                        pre.train.list, query.genes=NA, TrainDir, TaxDir, 
							prefix="", nlevel=nlevel, TaxFN=TaxFN)
         require(doMC)
         require(foreach)
         ...
         train.list = build_marker_index_tree_cl ( train.list, pre.train.list=pre.train.list, query.genes=NA,
                                                   ...
                                                   top.n.genes=top.n.genes, n.group.genes=n.group.genes, 
                                                   mc.cores=mc.cores, root.markers=root.markers)
      ...
      if (any(is.na(gdx)) || !is.null(rm.cl)) {
         print("### Alert! Some marker genes are missing in your data or you ")
         ...
         if (prebuild && file.exists(train.list$TaxFN)) {
            ...
            train.list = build_marker_index_tree_cl ( train.list, pre.train.list=pre.train.list, query.genes,
                                                      outdir=file.path(TaxDir, MI_str),
                                                      div_thr=3, subsample_pct=subsample_pct,
                                                      top.n.genes=top.n.genes, n.group.genes=n.group.genes, mc.cores=mc.cores, root.markers=root.markers)
            save(train.list, file=train.list$TaxFN)
...
predict_HKNN_cl <- function(lvl, iter_i, marker_index, query, assigned.df, children, ancestor, nodestr="", topk) {
   ...
   for (gg in groups) {
      ...
      if ( nn %in% names(marker_index) && 
           !any(is.na(marker_index[[nn]]$index_tree)) && 
           !any(is.na(marker_index[[nn]]$marker_tree)) &&
           !any(is.null(marker_index[[nn]]$marker_tree)) ) {
         if (is.null(children[[nn]]) || any(is.na(children[[nn]]))) {       # no children
...
look_up_ancestor <- function(cl.df, nlvl=3) {
   ...
   if (nlvl == 4) {
      if ("uberclass_label" %in% colnames(cl.df) && "class_label" %in% colnames(cl.df) && "subclass_label" %in% colnames(cl.df)) {
         ancestor = cl.df %>% select(uberclass_label, class_label, subclass_label, cl) %>% 
                        mutate(level1=uberclass_label, level2=class_label, level3=subclass_label, level4=cl) %>% 
                        select (level1, level2, level3, level4)
      }
      ancestor$level0 = rep("root", nrow(ancestor))
      out = ancestor[, c(5,1,2,3,4)]
   }
   if (nlvl == 3) {
   ...
   if (nlvl == 2) {
      if ("subclass_label" %in% colnames(cl.df)) {
         ancestor = cl.df %>% select(subclass_label, cl) %>% 
                        mutate(level1=subclass_label, level2=cl) %>% 
...
look_up_children <- function(cl.df, nlvl=3) {
   ...
   if (nlvl == 4) {
     cntr=0
     if ("uberclass_label" %in% colnames(cl.df) && "class_label" %in% colnames(cl.df)  && "subclass_label" %in% colnames(cl.df)) {
       # root / uberclass_label / class_label / subclass_label / cluster 
       nodestr = "root" 
       uberclasses = unique(cl.df$uberclass_label)
       children[[nodestr]] = uberclasses
       cntr=cntr+1
       for (nn in uberclasses) {
         nn.str = paste0(nodestr, ":", nn)
         #print(nn.str)
         nn.cl.df = cl.df %>% filter(uberclass_label == nn)
         nn.classes = unique(nn.cl.df$class_label)
         children[[nn.str]] = nn.classes
         cntr=cntr+1
         for (nn in nn.classes) {
           nn.str = paste0(nodestr, ":", nn)
           #print(nn.str)
   ...
   if (nlvl == 2) {
      if ("subclass_label" %in% colnames(cl.df)) {
         ...
         nodestr = "root" 
         groups = unique(cl.df$subclass_label)
         children[[nodestr]] = groups

         for (ss in groups) {
            ss.str = paste0(nodestr, ":", ss)
            #print(ss.str)
            ss.cl.df = cl.df %>% filter(subclass_label == ss)
                                   ...
                                   n.group.genes=3000, select.markers, cl.bin, outdir, root.markers=NULL )
{
   ...
   if (length(nn.group)==1  && length(nn.cl)==1) {
      ...
      if (file.exists(nn.markers.FN)) {
         ...
         if (length(query.genes)>1) nn.markers = intersect(nn.markers, query.genes )
         save(nn.markers, file=nn.markers.FN)
         save(nn.markers, file=nn.markers.level.FN)
      }
      #print("here") ; browser()
