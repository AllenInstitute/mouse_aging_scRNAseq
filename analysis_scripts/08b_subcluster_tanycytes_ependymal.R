## Description/Goal:
## Extract only tanycytes and ependymal cells and cluster them separately
...
## Create smaller cell-by-gene matrix with only tanycytes and ependymal cells
select.cells = anno.df.clean$sample_id[anno.df.clean$subclass_label %in% c("Tanycyte", "Epenedymal")]
...
## Cluster
resulti = iter_clust(norm.dat = norm.dat[,select.cells], select.cells = select.cells, "./data_tanepen/",
...
## Save for later
save(resulti, file = "./data_tanepen/resulti.rda")
save(merge.result, file = "./data_tanepen/merge.result.rda")